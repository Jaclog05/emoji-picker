...
export default function Home() {
  ...
  const handleClickEmojiOption = (emoji) => {
    ...
    inputRef.current.focus();
    inputRef.current.selectionStart = firstSection.length + emoji.length;
    ...
    <div className="w-1/2 mx-auto mt-20 min-h-full">
      <div className="flex w-full">
        <input
          ref={inputRef}
          className={`
            flex-1 placeholder:italic placeholder:text-slate-400 block bg-white w-1/2 
            border border-slate-300 rounded-tl-lg rounded-bl-lg py-2 pl-9 pr-3 shadow-sm 
        ...
        <button
          className={`
            flex-none rounded-tr-lg px-3 
            ${isDisplayed ? "bg-black" : ""}
          `}
          onClick={handleClick}
        >
          😄
      ...
      </div>
      {
        isDisplayed && (
          <EmojiPicker
            onEmojiClicked={handleClickEmojiOption}
          />
        )
      }
    </div>
